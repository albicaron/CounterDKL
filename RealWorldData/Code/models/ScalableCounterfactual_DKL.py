        ...
        self.covar_module = gpytorch.kernels.AdditiveStructureKernel(
            gpytorch.kernels.ScaleKernel(
                gpytorch.kernels.GridInterpolationKernel(
                    gpytorch.kernels.RBFKernel(ard_num_dims=1), grid_size=100, num_dims=1
                )
            ), num_dims=input_dim
        )
...
class DKLRegressionModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, hidden_layers, feature_extractor):
        ...
        self.covar_module = gpytorch.kernels.GridInterpolationKernel(
            gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel(ard_num_dims=hidden_layers[2])),
            num_dims=hidden_layers[2], grid_size=100
        )
