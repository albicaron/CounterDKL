...
class LargeFeatureExtractor(torch.nn.Sequential):
    def __init__(self, data_dim, hidden_layers):
        ...
        self.add_module('linear3', torch.nn.Linear(hidden_layers[1], hidden_layers[2]))


class GPClassificationModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, input_dim, num_classes):
        ...
        self.covar_module = gpytorch.kernels.AdditiveStructureKernel(
                ...
                gpytorch.kernels.RBFKernel(ard_num_dims=1, batch_shape=torch.Size((num_classes,))),
...
class MultitaskGPModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, n_causes, input_dim, num_classes):
...
class MultioutputGPModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, n_causes, n_out, input_dim, num_classes):
        ...
        self.covar_module = gpytorch.kernels.AdditiveStructureKernel(
            gpytorch.kernels.ScaleKernel(
                gpytorch.kernels.RBFKernel(ard_num_dims=input_dim, batch_shape=torch.Size((num_classes,))),
                batch_shape=torch.Size((num_classes,)),
            ), num_dims=input_dim
        )
...
class DKLClassificationModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, hidden_layers, feature_extractor, num_classes):
        super(DKLClassificationModel, self).__init__(train_x, train_y, likelihood)
        self.mean_module = gpytorch.means.ConstantMean(batch_shape=torch.Size((num_classes,)))
        self.covar_module = gpytorch.kernels.ScaleKernel(
                gpytorch.kernels.RBFKernel(ard_num_dims=hidden_layers[2], batch_shape=torch.Size((num_classes,))),
                batch_shape=torch.Size((num_classes,))
        )
...
class MultitaskDKLModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, hidden_layers, feature_extractor, n_causes, num_classes):
...
class MultioutputDKLModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, hidden_layers, feature_extractor, n_causes, n_out, num_classes):
...
class CounterGP:
    ...
    def __init__(self, train_x, train_a, train_y, input_dim, GPtype='single', GPU=True):
        ...
        self.train_y = torch.from_numpy(train_y).long().squeeze()
        self.GPU = GPU
        ...
        if self.GPtype is 'single':
            ...
            for j in range(self.n_out):
                for i in self.n_causes:
                    ...
                    if self.n_out == 1:
                        self.likelihood.append(gpytorch.likelihoods.DirichletClassificationLikelihood(
                            self.train_y[self.train_a == i],
                            learn_additional_noise=True))
                    elif self.n_out > 1:
                        self.likelihood.append(gpytorch.likelihoods.DirichletClassificationLikelihood(self.train_y[:, j][self.train_a == i],
                                                                                                      learn_additional_noise=True))

                    m = {'m_Y%s_A%s' % (j, i): GPClassificationModel(self.train_x[self.train_a == i],
                                                                     self.likelihood[count].transformed_targets,
                                                                     self.likelihood[count], self.data_dim,
                                                                     self.likelihood[count].num_classes)}

                    self.full_train_x.update({'m_Y%s_A%s' % (j, i): self.train_x[self.train_a == i]})

                    if self.n_out == 1:
                        self.full_train_y.update({'m_Y%s_A%s' % (j, i): self.train_y[self.train_a == i]})
                    elif self.n_out > 1:
        ...
        if self.GPtype is 'multitask':
            ...
            for j in range(self.n_out):
                ...
                self.likelihood.append(gpytorch.likelihoods.DirichletClassificationLikelihood(cat_train_y,
                                                                                              learn_additional_noise=True))

                m = {'m_Y%s' % j: MultitaskGPModel((cat_train_x, cat_train_i),
                                                   self.likelihood[j].transformed_targets, self.likelihood[j],
                                                   self.n_causes, self.data_dim, self.likelihood[j].num_classes)}
        ...
        if self.GPtype is 'multioutput':
            ...
            # initialize likelihood and model
            self.likelihood = gpytorch.likelihoods.DirichletClassificationLikelihood(cat_train_y,
                                                                                     learn_additional_noise=True)

            self.models = MultioutputGPModel((cat_train_x, cat_train_i,
                                             cat_train_k), self.likelihood.transformed_targets,
    ...
    def train(self, learn_rate=0.01, training_iter=100):
        """
        ...
        if self.GPtype is 'single':
            ...
            for i, j, k, likel in zip(self.models, self.full_train_x, self.full_train_y, self.likelihood):
                ...
                for n in range(training_iter):
                    ...
                    loss = -mll(output, likel.transformed_targets).sum()
                    loss.backward()
    ...
    def predict(self, x):
        ...
        if self.GPtype is 'single':
            ...
            for i, likel in zip(self.models, self.likelihood):
                ...
                observed_pred.append(probabilities.numpy())
            ...
            for j in range(self.n_out):
                for i in self.n_causes:
                    ...
                    with torch.no_grad():
                        pred = self.models(test_x, task_index, out_index)
...
class CounterDKL:
    ...
    def __init__(self, train_x, train_a, train_y, input_dim,
                 GPtype='single', hidden_layers=[50, 50, 2], GPU=True):
        """
        Initialize model
        ...
        if self.GPtype is 'single':
            ...
            for j in range(self.n_out):
                for i in self.n_causes:
                    ...
                    m = {'m_Y%s_A%s' % (j, i): DKLClassificationModel(self.train_x[self.train_a == i],
                                                                      self.likelihood[count].transformed_targets,
        ...
        if self.GPtype is 'multioutput':
            ...
            self.models = MultioutputDKLModel((cat_train_x, cat_train_i,
                                              ...
                                              self.n_causes, self.n_out, self.likelihood.num_classes)
